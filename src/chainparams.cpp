 ...
 */
class CGbtnParams : public CChainParams
{
public:
    CGbtnParams()
    {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        vSeeds.push_back(CDNSSeedData("bitcoinsv.io", "gbtn-seed.bitcoinsv.io", true));

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<uint8_t>(1, 25); // P2PKH addresses begin with B
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<uint8_t>(1, 68);  // P2SH  addresses begin with U
        base58Prefixes[SECRET_KEY] = std::vector<uint8_t>(1, 35);  // WIF   format begins with 2B or 2C
...
std::unique_ptr<CChainParams> CreateChainParams(const std::string &chain) {
    ...
    if (chain == CBaseChainParams::GBTN) {
        return std::unique_ptr<CChainParams>(new CGbtnParams());
    }
